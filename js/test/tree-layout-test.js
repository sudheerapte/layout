...
if (result !== null) { err(`error: ${result}`); }

expectedPre = [5, 2, 6, 7, 1, 3, 4];
log(JSON.stringify(treeLayout._pre));
if (! matchArray(treeLayout._pre, expectedPre)) {
  err(`Expected: ${JSON.stringify(expectedPre)}`);
} else {
  log(`preorder is correct`);
}
...
expectedOffsets.forEach( exp => {
  if (treeLayout._nodes[exp.id].offset !== exp.offset) {
    err(`expected offset did not match: ${JSON.stringify(exp)}`);
  }
});
log(`offsets are correct`);

// printOffsets(process.stdout, treeLayout._layers);

log(`--- test 2: 5,6,7 on right`);
nodeList = [
  {id: 1, children: [2, 3, 4], width: 20, height: 20},
...
log(`--- test 3: larger tree with missing node added later`);
...
result = treeLayout.setNodes(nodeList);
if (result !== 1) { err(`expected setNodes failure`); }
